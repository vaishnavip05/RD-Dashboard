...
(function (global) {
  ...
  function render(container, user) {
    ...
    var activeLabel = selectedNode ? selectedNode.label : (user.group ? user.group + ' — All' : 'ALL COLLEGES');
    html.push(
  ...
  function getVisibleRootNodes(user) {
    ...
    // 2. Dean / R&D Coordinator: sees only their assigned group and its departments
    ...
    // 3. HOD: sees only their assigned department
    if (user.role === 'hod') {
